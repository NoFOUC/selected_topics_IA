...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]): # -> torch.Tensor:
        ...
        source_padded = self.vocab.src.to_input_tensor(source, device=torch.device("mps"))   # Tensor: (src_len, b)
        target_padded = self.vocab.tgt.to_input_tensor(target, device=torch.device("mps"))   # Tensor: (tgt_len, b)
        
        source_padded.to(device=torch.device("mps"))
        target_padded.to(device=torch.device("mps"))
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]): # -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        ###         https://pytorch.org/docs/stable/generated/torch.html#torch.cat
        ###     Tensor Permute:
        ...
        enc_hiddens.to(torch.device("mps"))
        last_cell.to(torch.device("mps"))
        last_hidden.to(torch.device("mps"))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # explain in three sentence the effect of enc_masks on e_t
        # enc_masks is a tensor of shape (b, src_len) where b = batch size, src_len is maximum source length.
        # enc_masks is used to mask the attention scores e_t, so that the attention scores e_t is -inf where enc_masks has 1.
        # This is to prevent the decoder from attending to the padding tokens in the source sentence.
       
        # This is to prevent the decoder from attending to the padding tokens in the source sentence.
        # If the decoder attends to the padding tokens, it will affect the performance of the model.
        # For example, if the decoder attends to the padding tokens, the decoder will output the padding tokens as the translation.
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70): # -> List[Hypothesis]:
        ...
        src_encodings.to(torch.device('mps'))
